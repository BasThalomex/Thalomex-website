...
<section id="contact" class="section">
  ...
  <!-- Precision reticle — decorative, not content -->
  ...
  <div class="wrap">
    <p class="section-label fade-in">${content.contact.label}</p>
    <h2 class="fade-in">${content.contact.heading}</h2>
    <div class="contact-grid">
      <div class="contact-intro fade-in">
        ${content.contact.intro.map(function(p) { return '<p>' + p + '</p>'; }).join('')}
      </div>
      <div class="contact-details fade-in">
        ${content.contact.rows.map(function(r) {
          var val = r.link
            ? '<a href="' + r.link + '"' + (r.link.startsWith('http') ? ' target="_blank" rel="noopener"' : '') + '>' + r.value + '</a>'
            : '<span>' + r.value + '</span>';
          return '<div class="contact-row"><span class="lbl">' + r.label + '</span>' + val + '</div>';
        }).join('')}
      </div>
    </div>
  </div>
